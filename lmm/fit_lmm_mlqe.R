# Generic MLqE fitting for Gaussian linear mixed models
#
...
#
# The user also supplies the transformation family
#
#   tau(theta, r),
#
# which returns tau_r(theta). If theta_star is the original MLqE
# estimate, the consistency-corrected estimate is
#
#   theta_hat = tau(theta_star, q),
#
# while the forward transformation is
#
#   theta_star = tau(theta_hat, 1 / q).
#
# The corrected covariance matrix is obtained from the Jacobian of
# tau(theta, 1 / q), evaluated at the corrected estimate.
#
...
.lmm_mlqe_as_group_list <- function(value,
                                     name) {
  if (is.list(value)) {
    return(value)
  }
...
.lmm_mlqe_prepare_data <- function(Y,
                                    X,
                                    Z) {
  Y <- .lmm_mlqe_as_group_list(
    Y,
    "Y"
  )

  X <- .lmm_mlqe_as_group_list(
    X,
    "X"
  )

  Z <- .lmm_mlqe_as_group_list(
    Z,
    "Z"
  )
...
.lmm_mlqe_prepare_gamma_structure <- function(gamma_structure,
                                               m) {
...
.lmm_mlqe_default_start <- function(data,
                                     gamma_structure) {
  if (!requireNamespace(
    "MASS",
    quietly = TRUE
  ...
  gamma_information <-
    .lmm_mlqe_prepare_gamma_structure(
      gamma_structure =
        gamma_structure,
      m = data$m
    )
...
.lmm_mlqe_prepare_start <- function(start,
                                     p_beta) {
  if (!is.numeric(start) ||
...
.lmm_mlqe_call_Delta <- function(Delta,
                                  gamma,
                                  m,
                                  strict = FALSE) {
  output <- tryCatch(
...
.lmm_mlqe_compute_Delta_jacobian <- function(gamma,
                                              Delta,
                                              Delta_jacobian,
                                              m,
                                              method,
  ...
  if (!is.null(Delta_jacobian)) {
    ...
    delta_vector_function <- function(value) {
      ...
      delta_result <- .lmm_mlqe_call_Delta(
        Delta = Delta,
        gamma = value,
        m = m,
        strict = TRUE
...
.lmm_mlqe_block_matrix <- function(top_left,
                                    bottom_right) {
...
.lmm_mlqe_evaluate_model <- function(theta,
                                      data,
                                      structure,
                                      Delta,
                                      strict = FALSE) {
...
.lmm_mlqe_derivative_matrices <- function(theta,
                                           data,
                                           structure,
                                           Delta,
                                           Delta_jacobian,
  ...
  delta_derivative <-
    .lmm_mlqe_compute_Delta_jacobian(
      gamma = gamma,
      Delta = Delta,
      Delta_jacobian =
        Delta_jacobian,
...
.lmm_mlqe_compute_random_effects <- function(components,
                                              data) {
...
.lmm_mlqe_validate_tau_output <- function(value,
                                          parameter_names,
                                          label) {
...
.lmm_mlqe_call_tau <- function(tau_function,
                               theta,
                               r,
                               parameter_names,
                               label = "tau") {
...
.lmm_mlqe_compute_jacobian <- function(func,
                                       x,
                                       method,
                                       method_args,
                                       label) {
...
.lmm_mlqe_invert_matrix <- function(matrix_value,
                                    label) {
  inverse <- tryCatch(
    solve(
      matrix_value
...
.fit_lmm_mlqe_fixed <- function(Y,
  ...
  if (q < 1 &&
    ...
    stop(
      ...
      "defining the family tau_r(theta)."
    )
  }
  ...
  if (!is.null(tau) &&
      !is.function(tau)) {
    stop(
      "'tau' must be NULL or a function."
    )
  }
  ...
  structure <- .lmm_mlqe_prepare_start(
    start = start,
    p_beta = data$p.beta
  )
  ...
  # The supplied starting values are interpreted on the corrected
  # parameter scale. The optimizer works on the original MLqE scale:
  #
  #   theta_star = tau_{1/q}(theta).
  start_star <- .lmm_mlqe_call_tau(
    tau_function =
      tau_family,
    theta =
      start_corrected,
  ...
  # Validate both the corrected and original starting values.
  .lmm_mlqe_evaluate_model(
    theta =
      start_corrected,
    data = data,
  ...
  if (method %in% c(
    ...
    # Bounds refer to the original optimization parameterization.
    if (any(
      start_star <
      lower
    ) ||
      ...
      stop(
        "All original-scale starting values must lie within ",
  ...
  tau_q_jacobian <-
    .lmm_mlqe_compute_jacobian(
      ...
      method_args =
        jacobian_method_args,
      label =
        "The Jacobian of tau(theta_star, q)"
    )
  ...
  if (!all(
    ...
    stop(
      "The Jacobian of tau(theta_star, q) must be a ",
      p, " by ", p, " matrix."
    )
  }
  ...
  conditional_components_star <-
    .lmm_mlqe_compute_random_effects(
      components =
        fitted_components_star,
      data = data
    )
  ...
  result <- list(
    ...
    beta.star =
      fitted_components_star$beta,
    gamma =
      fitted_components$gamma,
    gamma.star =
      fitted_components_star$gamma,
    sigma2 =
      fitted_components$sigma2,
    sigma2.star =
      fitted_components_star$sigma2,
    Delta =
      fitted_components$Delta,
    Delta.star =
      fitted_components_star$Delta,
    standard.error =
      standard_error,
    standard.error.star =
      standard_error_star,
    ...
    K.star =
      K_star,
    Delta.jacobian.star =
      derivative_components_star$Delta.jacobian,
    mean.jacobians.star =
      derivative_components_star$mean.jacobians,
    covariance.jacobians.star =
      derivative_components_star$covariance.jacobians,
    F.star =
      derivative_components_star$F,
    tau.jacobian =
      tau_jacobian,
    ...
    fitted.values.star =
      fitted_components_star$means,
    marginal.fitted.values.star =
      fitted_components_star$means,
    conditional.fitted.values.star =
      conditional_components_star$conditional.means,
    random.effects.star =
      conditional_components_star$random.effects,
    residuals.star =
      fitted_components_star$residuals,
    marginal.residuals.star =
      fitted_components_star$residuals,
    conditional.residuals.star =
      conditional_components_star$conditional.residuals,
    covariance.matrices.star =
      fitted_components_star$covariance.matrices,
    covariance.inverses.star =
      fitted_components_star$covariance.inverses,
    log.density.star =
      fitted_components_star$log.densities,
    density.star =
      fitted_components_star$densities,
    objective =
      unname(
        optimization$value
      ),
...
fit_lmm_mlqe <- function(Y,
  ...
  if (!is.function(tau)) {
    stop(
      "For q = 'SQV', 'tau' must be supplied as a function ",
      "defining the family tau_r(theta)."
    )
  }
...
.select_q_lmm_mlqe_sqv <- function(Y,
                                    X,
                                    Z,
                                    Delta,
                                    tau,
  ...
  prepared_data <- .lmm_mlqe_prepare_data(
    Y = Y,
    X = X,
    Z = Z
  )
  ...
  start_structure <- .lmm_mlqe_prepare_start(
    start = start,
    p_beta = prepared_data$p.beta
  )

  start_corrected <- start_structure$start
  start <- start_corrected
  p <- start_structure$p
  # The starting vector is on the corrected parameter scale.
  # The fixed-q MLqE routine performs the q-dependent
  ...
  selected_fit$implementation.version <-
    "LMM-MLqE-SQV-original-algorithm-2026-07-30"
...
print.lmm_mlqe <- function(x,
                           ...
                           ...) {
  cat(
    "Gaussian linear mixed model fitted by MLqE\n"
  )
...
summary.lmm_mlqe <- function(object,
                             ...) {
  coefficient_table <- cbind(
    Estimate =
      object$coefficients
...
print.summary.lmm_mlqe <- function(x,
                                   digits = max(
                                     3L,
                                     getOption(
                                       "digits"
...
confint.lmm_mlqe <- function(object,
                             parm = seq_along(
                               object$coefficients
                             ),
                             level = object$level,
...
fitted.lmm_mlqe <- function(object,
                            type = c(
                              "marginal",
                              "conditional"
                            ),
...
residuals.lmm_mlqe <- function(object,
                               type = c(
                                 "marginal",
                                 "conditional"
                               ),
...
predict.lmm_mlqe <- function(object,
                             newdata = NULL,
                             type = c(
                               "mean",
                               "covariance",
  ...
  if (is.null(
    ...
    if (type == "mean") {
      return(
        if (corrected) {
          ...
          object$marginal.fitted.values.star
        }
      )
    }
    ...
    if (type == "covariance") {
      return(
        if (corrected) {
          ...
          object$covariance.matrices.star
        }
      )
    }
    ...
    return(
      if (corrected) {
        ...
        object$conditional.fitted.values.star
      }
    )
  }
  ...
  X_new <- .lmm_mlqe_as_group_list(
    newdata$X,
    "newdata$X"
  )

  Z_new <- .lmm_mlqe_as_group_list(
    newdata$Z,
    "newdata$Z"
  )
  ...
  if (is.null(
    ...
    stop(
      "'newdata$Y' is required for random-effects or ",
      "conditional-mean predictions."
    )
  }

  Y_new <- .lmm_mlqe_as_group_list(
    newdata$Y,
    "newdata$Y"
  )
  ...
  if (length(
    ...
    stop(
      "'newdata$Y', 'newdata$X' and 'newdata$Z' must contain ",
  ...
  for (i in seq_along(
    ...
    if (length(
      ...
      stop(
        "The response and design matrices have incompatible dimensions ",
        "in new group ",
        i,
        "."
      )
...
summary.lmm_mlqe_sqv <- function(object,
                                 ...) {
  result <- NextMethod(
    "summary"
  )
...
print.summary.lmm_mlqe_sqv <- function(x,
                                       digits = max(
                                         3L,
                                         getOption(
                                           "digits"
